...
public static class ExtendedColors
{
    ...
    public static Color GetColor(ExtendedColor color)
    {
        var field = typeof(ExtendedColors).GetField(color.ToString(), BindingFlags.Public | BindingFlags.Static);
        if (field != null && field.FieldType == typeof(Color))
        {
            return (Color)field.GetValue(null);
        }
        Debug.LogWarning($"[ExtendedColors] '{color}'에 해당하는 색상을 찾을 수 없습니다. 기본 색상으로 대체됩니다.");
        return Color.gray;
    }
}