...
public class BearGomStateSelectState : SubState
{
    ...
    public float[] phaseAttackPercents = new float[3] { 0.8f, 0.9f, 1.0f };   // 페이즈 별 공격 퍼센트

    public BearGomStateSelectState(BossStateMachine stateMachine, MainState parent) : base(stateMachine, parent)
    {
    }
    ...
    private void SelectStateInCombat(int currentPhaseIndex)
    {
        ...
        if (IsChangeAttackState(currentPhaseIndex))
        {
            ...
            // 공격 패턴을 뽑아냄
            attackPattern = SetPattern(currentAttackPatternList);

            // 공격 범위 설정
            if (attackPattern is AttackPattern)
                stateMachine.boss.attackDistacne = (attackPattern as AttackPattern).attackDistance;

            // 만약 공격 범위 안에 들어왔다면 look 상태 후 공격 상태로 돌입
            if (CheckAttackDistance())
            {
                parent.ChangeSubState(new BearGomLookState(stateMachine, parent, attackPattern));
                return;
            }
            // 만약 공격 범위 밖이라면 추격 상태로 전환 후 공격
            else
            {
                parent.ChangeSubState(new BearGomChaseState(stateMachine, parent, attackPattern));
                return;
            }
        }
